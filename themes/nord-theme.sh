...
################################################################################
# Script de Tema: Nord
# Descripción: Aplica la paleta de colores Nord (frío y minimalista)
################################################################################
...
echo -e "\n${CYAN}╔════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║${NC} Aplicando Tema: NORD"
echo -e "${CYAN}╚════════════════════════════════════════════════════╝${NC}\n"

# Colores del tema Nord
BG="#2e3440"
FG="#eceff4"
PRIMARY="#a3be8c"
SECONDARY="#bf616a"
...
if [[ -f "$KITTY_CONFIG" ]]; then
    ...
    # Colores específicos de Nord
    sed -i "s/color1.*/color1  #bf616a/" "$KITTY_CONFIG"
    sed -i "s/color2.*/color2  $PRIMARY/" "$KITTY_CONFIG"
    sed -i "s/color3.*/color3  #ebcb8b/" "$KITTY_CONFIG"
    sed -i "s/color4.*/color4  #81a1c1/" "$KITTY_CONFIG"
    sed -i "s/color5.*/color5  #b48ead/" "$KITTY_CONFIG"
    sed -i "s/color6.*/color6  $ACCENT/" "$KITTY_CONFIG"
...
if [[ -f "$DUNST_CONFIG" ]]; then
    ...
    sed -i '/\[urgency_critical\]/,/^\[/s/background = .*/background = "#2e3440"/' "$DUNST_CONFIG"
    sed -i '/\[urgency_critical\]/,/^\[/s/foreground = .*/foreground = "#bf616a"/' "$DUNST_CONFIG"
...
echo -e "\n${GREEN}╔════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║${NC} ✓ Tema Nord aplicado exitosamente"
echo -e "${GREEN}╚════════════════════════════════════════════════════╝${NC}\n"
